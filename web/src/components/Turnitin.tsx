...
type Busy = "upload" | "view-similarity" | "view-ai" | "download-similarity" | "download-ai" | "download-original" | "unlock" | null;
...
export function TurnitinScreen() {
  ...
  const start = async () => {
    ...
    try {
      const created = await createCheck(file);
  ...
  return (
    <section className="checker-page">
      ...
      {!result ? <div className="checker-card checker-upload-card">
        ...
        <div className="checker-upload-footer">
          <p><strong>Secure backend check</strong><span>The provider token is never placed in the website code or browser.</span></p>
          <button className="btn primary btn-large" disabled={!file || !availability?.ready || !availability.acceptingUploads || busy === "upload"} onClick={start}>{busy === "upload" ? "Uploading securely…" : "Check document"}</button>
        </div>
      </div> : <div className="checker-card checker-result-card">
        ...
        {!completed && !failed ? <div className="checker-processing" aria-live="polite">
          ...
          <p>The official status is refreshed every 5 seconds.</p>
          ...
          <p className="checker-note">Report links are refreshed from the provider before every view or download. No score or report is generated by ScholarlyWorks.</p>
        </>}
      </div>}
    </section>
  );
